...
def hist2pdf(data_path, output_dir, days_t=365.0, conf_t = 0.9, min_sup_t = 0.05, max_sup_t = 0.1, antecent=True):
    ...
    general_sample = [random.sample({k:v for (k,v) in entities_j.items() if v[1] > conf_t and (v[0] / days_t) > min_sup_t and (v[0] / days_t) < max_sup_t}.items(), 1)[0][1][3] for entity_i, entities_j in data.items() if len({k:v for (k,v) in entities_j.items() if v[1] > conf_t and (v[0] / days_t) > min_sup_t and (v[0] / days_t) < max_sup_t})>0]
    ...
    for entity_i, entities_j in data.items():
        ...
        for entity_j, histogram in entities_j.items():
            if ((histogram[0] / days_t) < min_sup_t) or ((histogram[0] / days_t) > max_sup_t) or (histogram[1] < conf_t):
                continue
