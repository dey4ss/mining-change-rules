...
def write_from_json_to_changes_file(json_filepath, changes_file, day):
    ...
    for entry in metadata_dict:
        ...
        columns_field_name = (
            str([x.encode("UTF8").decode("utf-8") for x in resource["columns_field_name"]])
            .encode("utf-8")
            .decode("utf-8")
        )
        columns_name = (
            str([x.encode("UTF8").decode("utf-8") for x in resource["columns_name"]]).encode("utf-8").decode("utf-8")
        )
        ...
        line = str(
            ...
            + str(createdAt)[:10]
            + ";"
            + str(metadata_updated_at)[:10]
            + ";"
            + str(data_updated_at)[:10]
            + ";"
            + str(columns_field_name)
            + ";"
            + str(columns_name)
            + "\n"
        )
...
def read_metadata_directory(change_path, output_file_path):
    ...
    changes_file.write(
        "day; id; name; updatedAt; createdAt; metadata_updated_at; data_updated_at; columns_field_name; columns_name\n"
    )
