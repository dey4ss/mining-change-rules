...
def parse_args():
    ...
    ap.add_argument("change_file", type=str, help="File with occurences per change (expect Python dict as .json)")
    ...
    ap.add_argument(
        "--num_bins", type=float, help=f"Bin count. Default {bin_default}", default=bin_default
    )
...
def main():
    ...
    actual_days = list(sorted({file_name[:10] for file_name in os.listdir(args["change_dir"]) if file_name.startswith("20")}))
    ...
    get_hist(all_changes, daily_changes, min_supp, args["max_supp"], args["min_conf"], actual_days, args["num_bins"])
